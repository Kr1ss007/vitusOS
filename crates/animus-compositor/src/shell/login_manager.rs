...
impl AELoginManager {
    ...
    /// Launches the desktop session directly (e.g. after successful authentication).
    ...
    pub fn authenticate_and_start(&self, password: &str) -> bool {
        ...
        let is_authenticated = crate::shell::pam::authenticate_user(&username, password);
...
mod tests {
    ...
    fn test_login_manager_session_start() {
        ...
        // Verify invalid password fails real PAM
        let auth = login.authenticate_and_start("invalid_test_password_123");
        assert!(!auth);
        assert!(!*login.is_session_active.read());

        // Test explicit session start lifecycle
        login.start_session();
        assert!(*login.is_session_active.read());
